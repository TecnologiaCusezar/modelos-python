...
def each(df: pandas.DataFrame, columnName: str, callback: pandas.Scalar | pandas.Sequence | set | pandas.Mapping | None) -> pandas.DataFrame:
...
def resolve_env(key: str, default: str) -> str|None:
    #
    ...
    #
    value = env(key[len(env_token)])
    if env_token in key and value != None:
        return value
    else:
        return default
